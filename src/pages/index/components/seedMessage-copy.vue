...
const messageTemplateFields = computed(() => {
  ...
  if (!form.value.templateId || isEmpty(records)) return []
  return records.filter((item) => item.template_id__c === form.value.templateId)
})
...
const updateFormModel=()=>{
  ...
  for (let i = 0; i < fields.length; i++) {
    const {correspondingFieldName__c,dynamicParameter__c,order__c,format__c,valueType__c} = fields[i];
    ...
    if(correspondingFieldName__c) {
      if(isDate){
        form.value[key] = value ? dayjs(toNumber(value)).format(format__c) : ''
      }else {
        form.value[key] = value ? value.toString():''
      }
    }else {
      form.value[key] = ''
    }
...
watch(()=>model.value.template,newTemplateId=>{
  if (isEmpty(messageTemplateFields.value)||!newTemplateId) return
    await run({
      data: {
        xoql: `select ${messageTemplateFields.join(
          ','
...
onMounted(async () => {
  ...
  if (!isEmpty(messageTemplateOptions.value)) {
    model.value.templateId = get(messageOptions.value, `[0].value`)
  }
})
</script>
<template>
